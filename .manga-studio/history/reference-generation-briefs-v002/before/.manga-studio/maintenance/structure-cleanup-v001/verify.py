...
assert config['workflow_phase'] == 'phase_5_story_locked_canon_v002_correction_proposed_storyboard_blocked'
...
assert all(config['stage_lock_records'][key] == f'.manga-studio/locks/{key}-v001.json' for key in expected_true_locks)
...
assert config['active_canon_version'] == '.manga-studio/canon/versions/chapter-001-canon-v001.json'
...
assert authority['active_canon_sha256'] == '8e99db551f9eda9f3b56116b15cd6425a807d27b439df3a493864385612cfb58'
...
assert authority['canon_approval_relative_path'] == '.manga-studio/approvals/approval-84d84d30ba764d1b845e80a28d88a628.json'
...
assert authority['reference_generation_briefs_version'] == 'v001'
assert authority['reference_generation_briefs_decision_relative_path'] == '.manga-studio/decisions/chapter-001-reference-generation-briefs-v001.json'
assert authority['reference_generation_release_status'] == 'deferred'
assert len(brief_paths) == 8 and all(safe(rel).is_file() for rel in brief_paths)
assert authority['proposed_canon_version'] == '.manga-studio/canon/versions/chapter-001-canon-v002.json'
assert authority['proposed_canon_status'] == 'correction_proposed_review_ready'
assert authority['proposed_canon_sha256'] == '897ed992c1106ee39b10f903019a4eb24fd9a51309cc69960e7505d7c9365725'
assert sha(safe(authority['proposed_canon_version'])) == authority['proposed_canon_sha256']
assert read(safe(authority['proposed_canon_version']))['status'] == 'proposed'
...
assert authority['storyboard_status'] == 'blocked_pending_canon_v002_approval_and_relock'
...
assert sha(WORK / 'canon/versions/chapter-001-canon-v001.json') == authority['active_canon_sha256']
...
result = {'record_type': 'structure_cleanup_validation', 'project_id': PID, 'checked_at': datetime.now(timezone.utc).isoformat(), 'status': 'pass',
          ...
          'warnings': ['Canon v002 correction approval and controlled canon/story relock remain pending; storyboard drafting is paused.', 'Seven existing editorial warnings remain recorded; the manuscript-activation portion of SMA-WARN-006 is resolved.', 'Eight deferred environment, prop and temporal-phenomenon generation briefs are prepared, but their visual outputs and supporting-character references remain pending.', 'The inventory records import coordinates, not a fresh root scan. Use provenance and the relocation manifest for current storage.', 'Historical reports and evidence keep original paths and require hash-aware resolution.', 'Entity-specific folders and unreleased page prompts intentionally differ from the reference story.']}
if args.record:
