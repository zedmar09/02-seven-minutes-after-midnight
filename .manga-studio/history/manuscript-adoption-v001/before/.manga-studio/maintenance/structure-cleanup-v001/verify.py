...
assert meta['active'] is False and meta['status'] == 'draft_inactive' and meta['adoption_approval'] is None
...
for key in ['stage_locks', 'stage_lock_records', 'active_manuscript_version', 'active_canon_version', 'active_storyboard_version', 'image_generation_enabled', 'workflow_phase']:
...
actual = file_set(ROOT / 'manga')
assert actual == set(archive['expected_working_files'])
...
for rel in sorted(actual | {'README.md'}):
    ...
    for href in re.findall(r'\[[^\]]+\]\(([^)]+)\)', content):
        ...
        links += 1
assert not any(p.suffix.lower() in {'.png', '.webp', '.jpg', '.jpeg'} for p in (ROOT / 'manga').rglob('*'))
...
result = {'record_type': 'structure_cleanup_validation', 'project_id': PID, 'checked_at': datetime.now(timezone.utc).isoformat(), 'status': 'pass',
          ...
          'approvals_preserved': len(approvals), 'draft_sha256': expected, 'exact_approved_edits_verified': len(register['edits']), 'strict_source_locators_verified': locator_count,
          ...
          'locks_changed': False, 'active_versions_changed': False, 'artwork_created': False, 'production_ready': False,
          'warnings': ['Manuscript adoption and preproduction remain pending.', 'Seven existing editorial warnings remain; this cleanup does not resolve them.', 'The inventory records import coordinates, not a fresh root scan. Use provenance and the relocation manifest for current storage.', 'Historical reports and evidence keep original paths and require hash-aware resolution.', 'Entity-specific folders and unavailable images/page prompts intentionally differ from the reference story.']}
if args.record:
