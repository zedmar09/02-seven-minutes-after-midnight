...
Audit strictly in numerical order.

A local textual PASS applies only to that strip and its explicitly checked seam-out. It does not approve the next strip, missing canonical WebPs, or rendered pixels.
...
Only after formal hardening and canonical approval, generate strictly:
`001 → 002 → ... → 036`.
